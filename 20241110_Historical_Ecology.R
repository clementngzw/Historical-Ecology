...
overall_bcp_plot %>% filter(posterior.prob > 0.75)

combined_overall %>% filter(Country == "Malaysia") %>% summarise(max(Harvest, na.rm = T))
combined_overall %>% filter(Country == "Singapore") %>% summarise(max(Harvest, na.rm = T))
...
plot6 <- combined_overall %>% 
  ...
  scale_y_continuous(name = 'Landings (thousand tonnes)', sec.axis = sec_axis(~. / 10, name = "Landings in Singapore (thousand tonnes)"))+
  scale_x_continuous(limits=c(1928, 2021), breaks = c(1930,1950,1970,1990,2010), labels = c(1930,1950,1970,1990,2010))+
...
## State catch
unique(state_catch$Grouping_Name)
state_catch_aggregate <- state_catch %>% 
  mutate(groups = case_when(Grouping_Name == "Crabs" | Grouping_Name == "Lobsters" | Grouping_Name == "Mantis Shrimp" | Grouping_Name == "Miscellaneous marine crustacaens" | Grouping_Name == "Other prawns" | Grouping_Name == "Penaeid prawns" | Grouping_Name == "Shrimps, prawns, etc." | Grouping_Name == "Tiger prawn" ~ "Crustaceans", 
                            Grouping_Name == "Chondrichthyes" | Grouping_Name == "Rays" | Grouping_Name == "Sharks" ~ "Chondrichthyes",
                            Grouping_Name == "Cuttlefishes" | Grouping_Name == "Octopodidae" | Grouping_Name == "Squids" ~ "Cephalopods",
                            Grouping_Name == "Holothuroidea" | Grouping_Name == "Horseshoe Crab" | Grouping_Name == "Jellyfishes" | Grouping_Name == "Sea turtles" | Grouping_Name == "Shellfish" | Grouping_Name == "Spongidae" ~ "Others",
                            Grouping_Name == "Trash Fish" ~ "Trash Fish",
                            TRUE ~ "Teleosts"))
state_catch_aggregate <- aggregate(Harvest ~ Year + State + groups, state_catch_aggregate, sum) 
state_catch_aggregate$Harvest <- round(state_catch_aggregate$Harvest, 8)

taxon <- c("Cephalopods","Chondrichthyes","Crustaceans","Teleosts","Trash Fish","Others")
state_catch_aggregate <- arrange(transform(state_catch_aggregate, Classification=factor(groups,levels=taxon)), Classification)

state_catch_aggregate %>% 
  mutate(State = case_when(State == "West Johor" ~ "Johore West", 
                           State == "East Johor" ~ "Johore East", 
                           T ~ State)) %>%
  mutate(State = case_when(State == "Penang and Province Wellesley" ~ "Penang and\nProvince Wellesley", 
                           T ~ State)) %>%
  ggplot(aes(x = Year, y = Harvest))+
  geom_area(aes(fill = Classification), colour='black', linewidth = 0.025, position = "stack", stat = "align")+
  facet_wrap(.~State, ncol = 4, scales = 'free') +
  scale_x_continuous(breaks = c(1920,1940,1960,1980,2000,2021), labels = c(1920,1940,1960,1980,2000,2021))+
  scale_y_continuous(name='Landings (thousand tonnes)', labels = scales::comma, limits = c(0, NA))+
...
## Set threshold for posterior probability as 0.75
overall_bcp_plot %>% filter(posterior.prob > 0.75)
...
species_data <- species_data %>%
  ...
  subset(select = -c(posterior.prob)) %>%
  drop_na() %>%
  subset(select = -c(State, Grouping_Name, values, oil_prices)) 

species_data <- species_data %>%
  filter(var == "Perlis_Rastrelliger spp." | var == "Kedah_Rastrelliger spp." | var == "Perak_Rastrelliger spp.") %>%
  filter((var == "Kedah_Rastrelliger spp." & year >= 1968 & year < 1998) | 
           (var == "Perak_Rastrelliger spp." & year >= 1968 & year < 2017) | 
           (var == "Perlis_Rastrelliger spp." & year >= 1968 & year < 1998)) 

species_data <- species_data %>%
  group_by(var) %>% 
  mutate(c_Powered = Powered * cumprod(rep(1 + 2 / 100, n()))) %>%
  mutate(c_nPowered = `Non-powered` * cumprod(rep(1 + 2 / 100, n()))) %>%
  ungroup()

species_data <- species_data %>%
  group_by(var) %>% 
  mutate(cpue = catch / c_Powered) %>%
  ungroup()

species_data <- species_data %>% 
  mutate(State = case_when(var == "Perak_Rastrelliger spp." ~ "Perak",
                           var == "Kedah_Rastrelliger spp." ~ "Kedah",
                           var == "Perlis_Rastrelliger spp." ~ "Perlis",
                           FALSE ~ "others"))
rastrelliger_order <- c("Perak","Kedah","Perlis")
species_data <- arrange(transform(species_data, State=factor(State,levels=rastrelliger_order)),State)
...
grangertest(catch ~ fishermen, order = 2, data = pe_rs) 
grangertest(catch ~ c_Powered, order = 2, data = pe_rs) 
grangertest(catch ~ c_nPowered, order = 2, data = pe_rs)

grangertest(fishermen ~ catch, order = 2, data = pe_rs) 
grangertest(c_Powered ~ catch, order = 2, data = pe_rs) 
grangertest(c_nPowered ~ catch, order = 2, data = pe_rs) 
...
grangertest(catch ~ fishermen, order = 1, data = kd_rs) 
grangertest(catch ~ c_Powered, order = 1, data = kd_rs) 
grangertest(catch ~ c_nPowered, order = 1, data = kd_rs)

grangertest(fishermen ~ catch, order = 1, data = kd_rs) 
grangertest(c_Powered ~ catch, order = 1, data = kd_rs) 
grangertest(c_nPowered ~ catch, order = 1, data = kd_rs) 
...
grangertest(catch ~ fishermen, order = 1, data = pr_rs) 
grangertest(catch ~ c_Powered, order = 1, data = pr_rs) 
grangertest(catch ~ c_nPowered, order = 1, data = pr_rs)

grangertest(fishermen ~ catch, order = 1, data = pr_rs) 
grangertest(c_Powered ~ catch, order = 1, data = pr_rs) 
grangertest(c_nPowered ~ catch, order = 1, data = pr_rs) 

######################################################################################################
######################################################################################################

### Identify the most important variables in Rastrelliger harvest patterns
# Perlis - Powered vessels (p = 0.01425) affected landings
# Kedah - Powered boats (p = 0.00323) affected landings 
# Perak - None

## Case examples for specific species across space and time
plot4a <- catch_data %>% 
  filter(var == "Kedah_Rastrelliger spp." & year >= "1968" & year <= "1997") %>% 
  ggplot(aes(x=year, y=values))+
  geom_point(size=1.5, shape=15)+
  geom_smooth(stat='smooth')+
  scale_x_continuous(limits=c(1965,2000), breaks=c(1965,1970,1975,1980,1985,1990,1995,2000), labels=c(1965,1970,1975,1980,1985,1990,1995,2000), name='Year')+
  scale_y_continuous(name='Landings (thousand tons)', limits = c(0,30), breaks = c(0, 10, 20, 30))+
  theme(plot.title = element_text(hjust = 0.5, size=22),
        axis.text = element_text(size=18),
        axis.title = element_text(size=20),
        strip.text.y = element_text(size = 10),
...
plot4b <- boat_type %>%
  filter(Type == "Powered" & State == "Kedah") %>%
...
plot4c <- catch_data %>% 
  filter(var == "Perlis_Rastrelliger spp." & year >= "1968" & year <= "1997") %>% 
  ggplot(aes(x=year, y=values))+
  geom_point(size=1.5)+
  geom_smooth(stat='smooth')+
  scale_x_continuous(limits=c(1965,2000), breaks=c(1965,1970,1975,1980,1985,1990,1995,2000), labels=c(1965,1970,1975,1980,1985,1990,1995,2000), name='Year')+
  scale_y_continuous(name='Landings (thousand tons)', limits = c(-0.2,30), breaks = c(0, 10, 20, 30))+
  theme(plot.title = element_text(hjust = 0.5, size=22),
        axis.text = element_text(size=18),
        axis.title = element_text(size=20),
        strip.text.y = element_text(size = 10),
...
scale1 = species_data %>% filter(var == "Kedah_Rastrelliger spp.") %>% summarise(max(c_Powered)) / species_data %>% filter(var == "Kedah_Rastrelliger spp.") %>% summarise(max(catch))
scale1 = scale1[1,1]
plot4e <- ggplot()+
  geom_point(data = species_data %>% filter(var == "Kedah_Rastrelliger spp."), aes(x=year, y=catch, colour = 'Landings'), size=2, shape=15)+
  geom_smooth(data = species_data %>% filter(var == "Kedah_Rastrelliger spp."), aes(x=year, y=catch), stat='smooth', colour = '#000000', se = T)+ 
  
  geom_point(data = species_data %>% filter(var == "Kedah_Rastrelliger spp."), aes(x=year, y=c_Powered/scale1, colour = 'Powered boats'), size=2, shape=17)+
  geom_smooth(data = species_data %>% filter(var == "Kedah_Rastrelliger spp."), aes(x=year, y=c_Powered/scale1), stat='smooth', colour = '#000000', se = T)+ 
...
scale2 = species_data %>% filter(var == "Perlis_Rastrelliger spp.") %>% summarise(max(c_Powered)) / species_data %>% filter(var == "Perlis_Rastrelliger spp.") %>% summarise(max(catch))
scale2 = scale2[1,1]
plot4f <- ggplot()+
  geom_point(data = species_data %>% filter(var == "Perlis_Rastrelliger spp."), aes(x=year, y=catch, colour = 'Landings'), size=2, shape=15)+
  geom_smooth(data = species_data %>% filter(var == "Perlis_Rastrelliger spp."), aes(x=year, y=catch), stat='smooth', colour = '#000000', se = T)+ 
  
  geom_point(data = species_data %>% filter(var == "Perlis_Rastrelliger spp."), aes(x=year, y=c_Powered/scale2, colour = 'Powered boats'), size=2, shape=17)+
  geom_smooth(data = species_data %>% filter(var == "Perlis_Rastrelliger spp."), aes(x=year, y=c_Powered/scale2), stat='smooth', colour = '#000000', se = T)+ 
...
plot4 <- ggpubr::ggarrange(plot4f, plot4e, labels = c("A", "B"), ncol = 1, common.legend = TRUE, legend = 'right', vjust = 0.4, hjust = -6) +
  theme(plot.margin = margin(1, 1, 1, 1, "cm"))
plot4 # top figure is Perlis, bottom figure is Kedah

species_data %>% 
  filter(State == "Perak") %>% 
  summarise(years = n(),
            f_mean = mean(fishermen, na.rm = TRUE), f_sd = sd(fishermen, na.rm = TRUE),
            p_mean = mean(Powered, na.rm = TRUE), p_sd = sd(Powered, na.rm = TRUE),
            np_mean = mean(`Non-powered`, na.rm = TRUE), np_sd = sd(`Non-powered`, na.rm = TRUE)) 
...
### Example of Lobsters
catch_cp <- catch_data %>%
  filter(posterior.prob >= 0.75 & (State == "Kelantan" | State == "Terengganu" | State == "Pahang" | State == "Johore East")) %>%
  arrange(State, Grouping_Name)
...
plot5a <- catch_data2 %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5, size=22),
        ...
        legend.text=element_text(size=14),
        panel.spacing = unit(1, "lines"),
        plot.margin = margin(1.5, 1.5, 1.5, 2.85, "cm"))
plot5a
...
plot5b <- catch_data2 %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5, size=22),
        ...
        legend.text=element_text(size=14),
        panel.spacing = unit(1, "lines"),
        plot.margin = margin(0, 1.5, 1.5, 2.5, "cm"))
plot5b
...
base_map <- ggmap(spatial_Malaya) + 
  ...
  geom_segment(aes(x = 100.5, y = 5.7, xend = 99.5, yend = 5.6), color = "black", size = 0.5, alpha = 0.8) + 
  ...
  annotate('text', y=5.6, x=98.2, color='grey15', size=5, label='Kedah', fontface = 2) +
...
ggdraw()+
  draw_plot(base_map)+
  draw_plot(world_map, height=0.200, x=0.215, y=0.130) 
